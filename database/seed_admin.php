...
if ($stmt->num_rows > 0) {
    // Admin already exists — update the password hash in case it changed
    $stmt->close();
    $update = $conn->prepare('UPDATE admins SET password = ? WHERE username = ?');
    $update->bind_param('ss', $hashed_password, $username);
    if ($update->execute()) {
        echo '<p style="color:green;">✔ Admin account already exists. Password hash updated successfully.</p>';
    } else {
        echo '<p style="color:red;">✘ Failed to update admin: ' . htmlspecialchars($conn->error) . '</p>';
    }
    $update->close();
} else {
    ...
    if ($insert->execute()) {
        ...
        echo '<p style="color:red;">✘ Failed to create admin: ' . htmlspecialchars($conn->error) . '</p>';
    }
    $insert->close();
}

echo '<p><strong>Username:</strong> ' . htmlspecialchars($username) . '</p>';
echo '<p><em>Password is stored as a bcrypt hash — never in plain text.</em></p>';
